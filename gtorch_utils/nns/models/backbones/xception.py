# -*- coding: utf-8 -*-
"""
gtorch_utils/nns/models/backbones/xception.py

Source https://github.com/YudeWang/semantic-segmentation-codebase/blob/main/lib/net/backbone/xception.py
...
class SeparableConv2d(nn.Module):
    def __init__(
            ...
            activate_first=True, inplace=True, norm_layer=nn.BatchNorm2d
    ):
        super().__init__()
...
class Block(nn.Module):
    def __init__(
            self, in_filters, out_filters, strides=1, atrous=None, grow_first=True, activate_first=True,
            inplace=True, norm_layer=nn.BatchNorm2d
    ):
        super().__init__()
        self.norm_layer = norm_layer
        if atrous == None:
...
def xception(pretrained=True, os=8, **kwargs):
    ...
    if pretrained:
        old_dict = torch.hub.load_state_dict_from_url(MODEL_URLS['xception'], model_dir=DIR_CHECKPOINTS)
