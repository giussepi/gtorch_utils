...
from gtorch_utils.datasets.segmentation.datasets.consep.utils.augmentations import (
    add_to_brightness,
    add_to_contrast,
    add_to_hue,
    add_to_saturation,
...
class OnlineCoNSePDataset(torch.utils.data.Dataset):
    """
    CoNSeP dataset for .npy files created using gtorch_utils.datasets.segmentation.datasets.consep.utils.patches.patches -> ProcessDataset class.
    Loads images from a file list and performs online data augmentation with the imgaug library.
    ...
    Usage:
        ...
        input_dataset = OnlineCoNSePDataset(
            ...
            crop_img_shape=model_crop_img_shape,
            crop_mask_shape=model_outut_shape,
            mode=DB.TRAIN,
    ...
    def __init__(self, **kwargs):
        ...
        self.crop_img_shape = kwargs.get('crop_img_shape')
        self.crop_mask_shape = kwargs.get('crop_mask_shape')
    ...
    def __get_augmentation(self, mode, rng):
        if mode == DB.TRAIN:
            shape_augs = [
                ...
                iaa.CropToFixedSize(
                    self.crop_img_shape[0], self.crop_img_shape[1], position="center"
                ),
    ...
    def __getitem__(self, idx):
        ...
        img = cropping_center(img, self.crop_img_shape)
        feed_dict = {"img": img}
        inst_map = (ann[..., 0]).copy()  # HW1 -> HW
        inst_map[inst_map > 0] = 1
        feed_dict["mask"] = cropping_center(inst_map, self.crop_mask_shape)
